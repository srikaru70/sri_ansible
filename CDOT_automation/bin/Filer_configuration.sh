...
confirm_input_LSM(){
             ...
             if [ -f "$GLOBAL_VAR/"$cluster"_LS_mirror_configuration.yml" ] ; then
                 cat $GLOBAL_VAR/"$cluster"_LS_mirror_configuration.yml
                 echo ""
                 confirm_input
             else
                 echo "Please update the Control file at $GLOBAL_VAR/"$cluster"_LS_mirror_configuration.yml "
                 exit 0
             fi
}
...
LSM_volume_creation(){
             #  read -p "Enter New Vserver Name:" cluster
               ansible-playbook --extra-vars \
               "cluster=$cluster \
               username=$username \
               password=$password  \
               vserver=$vserver " \
               --extra-vars @$GLOBAL_VAR/"$cluster"_LS_mirror_configuration.yml \
               $SRC_YAML/Cluster_config.yml -t LSM_vol_Create
               echo "Please wait for LS volume(s) to be online( ONE Minitue)"
}
...
LS_Mirror_Creation(){
              ...
              vserver=$vserver " \
              --extra-vars @$GLOBAL_VAR/"$cluster"_LS_mirror_configuration.yml \
              $SRC_YAML/Cluster_config.yml -t LSM_Initialize
...
#    ansible-playbook --extra-vars="@$GLOBAL_VAR/"$cluster"_aggregate_configuration.yml" $SRC_YAML/Cluster_config.yml -t Aggregate_create
     ...
     $SRC_YAML/Cluster_config.yml -t Aggregate_create
}
...
show_menus() {
    ...
    echo ""
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo " Welcome to Netapp filer Configuration !"
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
read_options(){
    ...
    case $choice in
        ...
        5) LIF_Creation ;;
        6) SP_configuration ;;
        7) echo "" ;echo "~~~~~~~~~~~Thanks for using Filer Automation WorkFlow !!"; \
           echo "" ;exit ;;
...
LSM_Creation(){
   Cluster_login
   read -p "Enter Vserver(LSM to be Initialized) Name:" vserver
   confirm_input_LSM
   LSM_volume_creation
   LS_Mirror_Creation
   break
